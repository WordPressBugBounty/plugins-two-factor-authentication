		...
		if (is_wp_error($user)) {
			...
			if (is_wp_error($code_ok)) {
				...
				$error_message = apply_filters(
					'simba_tfa_message_code_incorrect',
					__('The one-time password (TFA code) you entered was incorrect.', 'two-factor-authentication') . $additional
				);
				$ret =  new WP_Error('authentication_failed', '<strong>'.__('Error:', 'two-factor-authentication').'</strong> ' . $error_message);
				if (is_a($user, 'WP_User')) $this->log_incorrect_tfa_code_attempt($user);

				// Handle TFA errors on the EDD login form.
				if (!empty($params['edd_action']) && 'user_login' === $params['edd_action'] && function_exists('edd_set_error')) {
					edd_set_error(
